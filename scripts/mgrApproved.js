...
function processData(data){
    ...
    document.getElementById("hroCount").innerHTML = table7.rows( {search:'applied'} ).count();
    ...
    //Approve Row
    $('#hro tbody').on('click', 'i.editRow2', function () {
        ...
        for(var i = 0; i < rowDB["data"].length; i++) {
            if ((rowDB["data"][i].promStatus == "Approved by Manager under Spec. Cons." || rowDB["data"][i].promStatus == "Approved by Manager with Justification" || rowDB["data"][i].promStatus == "Approved by Manager" || rowDB["data"][i].promStatus == "Declined at Challenge Session" || (rowDB["data"][i].promStatus == "Approved at Challenge Session with Justification" || rowDB["data"][i].promStatus == "Approved at Challenge Session")) && rowDB["data"][i].sortHRO == currentRowCollection["HRO"] && rowDB["data"][i].staffCat == "Staff") {
                nCount = nCount + 1;
            }
            else{}
        }
        for(var i = 0; i < rowDB["data"].length; i++) {
            if ((rowDB["data"][i].promStatus == "Approved at Challenge Session under Spec. Cons." || rowDB["data"][i].promStatus == "Approved at Challenge Session with Justification" || rowDB["data"][i].promStatus == "Approved at Challenge Session" || rowDB["data"][i].promStatus == "Declined at Challenge Session") && rowDB["data"][i].sortHRO == currentRowCollection["HRO"] && rowDB["data"][i].staffCat == "Staff") {
                mCount = mCount + 1;
            }
            else{}
        }
        ...
        if (currentRowCollection["promStatus"] == "Approved by Manager"){
            ...
            if ((1 + mCount) == nCount) {
                ...
                var to = "";
                for(var i = 0; i < rowDB["data"].length; i++) {
                    if (rowDB["data"][i].sortHRO == currentRowCollection["HRO"] && rowDB["data"][i].staffCat == "Staff" && (rowDB["data"][i].promStatus == "Approved by Manager under Spec. Cons." || rowDB["data"][i].promStatus == "Approved by Manager with Justification" || rowDB["data"][i].promStatus == "Approved by Manager" || rowDB["data"][i].promStatus == "Approved at Challenge Session under Spec. Cons." || rowDB["data"][i].promStatus == "Approved at Challenge Session with Justification" || rowDB["data"][i].promStatus == "Approved at Challenge Session")) {
                        sNo = sNo + 1;
                ...
                for(var n = 0; n < current["usersData"].length; n++) {
                    if (current["usersData"][n].accessProfile == "HRO Line" && current["usersData"][n].sortHRO == currentRowCollection["HRO"].slice(0, -1)) {
            ...
            alert("Staff has been approved for promotion with Justification");
            location.reload();
        }
        else if (currentRowCollection["promStatus"] == "Approved by Manager under Spec. Cons."){
            ...
            if ((1 + mCount) == nCount) {
                ...
                var cc = 'Cc: ' + current["user"] + '@nlng.com, ' + cc2 + ', ' + cc3 + ', ' + cc4;
                var subject = "Promotion List Approved under Spec. Cons.";
                var txt = 'Dear HRT Focal' + '\n\nThe approved list with the underlisted staff has been processed by ' + titleCase(current["user"]) + '.\n\n' + myTable + '\n\nVisit the link below to review. \n\n' + 'http://wapp-bny.nlng.net/datacentric/promotion/approvedList.html \n\n' + 'Regards\nTalent Management';
                sendMail (to,subject,txt,cc);
            }
            alert("Staff has been approved for promotion");
    ...
    $('#hro tbody').on('click', 'i.editRow3', function () {
    ...
    $("#hro").on('mousedown', "select", function(e) {
        e.stopPropagation();
    });

///////////// Load page based on Role
      
    if (rowCollection["access"] == "HRO") {
    	$('#homepage').show();
        $('#error').hide();
        $('#PP1').show();
        $('#tab1').show();
...
function addReason() {
    ...
    var to = "";
    for(var i = 0; i < rowDB["data"].length; i++) {
        if ((rowDB["data"][i].promStatus == "Approved by Manager under Spec. Cons." || rowDB["data"][i].promStatus == "Approved at Challenge Session under Spec. Cons." || rowDB["data"][i].promStatus == "Approved by Manager with Justification" || rowDB["data"][i].promStatus == "Approved at Challenge Session with Justification" || rowDB["data"][i].promStatus == "Approved by Manager" || rowDB["data"][i].promStatus == "Declined at Challenge Session" || rowDB["data"][i].promStatus == "Approved at Challenge Session") && rowDB["data"][i].sortHRO == currentRowCollection["HRO"] && rowDB["data"][i].staffCat == "Staff") {
            nCount = nCount + 1;
        }
        else{}
    }
    ...
    if (document.getElementById("gmReason").value == ""){
        ...
        if ((1 + mCount) == nCount) {
            ...
            var to = "";
            for(var i = 0; i < rowDB["data"].length; i++) {
                if (rowDB["data"][i].sortHRO == rowCollection["HRO"] && rowDB["data"][i].staffCat == "Staff" && (rowDB["data"][i].promStatus == "Approved at Challenge Session under Spec. Cons." || rowDB["data"][i].promStatus == "Approved at Challenge Session with Justification" || rowDB["data"][i].promStatus == "Approved at Challenge Session")) {
                    sNo = sNo + 1;
                    var newPers = sNo + '. ' + rowDB["data"][i].persName + ' (' + rowDB["data"][i].refIndic + ')\n';
                    myTable = myTable + newPers;
                }
            ...
            var cc = 'Cc: ' + current["user"] + '@nlng.com, ' + cc2 + ', ' + cc3 + ', ' + cc4;
            var subject = "Promotion List Approved";
            var txt = 'Dear HRT Focal' + '\n\nThe approved list for ' + rowCollection["dept"] + ' departments with the underlisted staff has been processed by ' + titleCase(current["user"]) + '.\n\n' + myTable + '.\n\nVisit the link below to review. \n\n' + 'http://wapp-bny.nlng.net/datacentric/promotion/approvedList.html \n\n' + 'Regards\nTalent Management';
            sendMail (to,subject,txt,cc)
        }
...
function gmApprove() {
    ...
    var mCount = 0;
    for(var i = 0; i < rowDB["data"].length; i++) {
        if ((rowDB["data"][i].promStatus == "Approved by Manager under Spec. Cons." || rowDB["data"][i].promStatus == "Approved by Manager with Justification" || rowDB["data"][i].promStatus == "Approved by Manager" || rowDB["data"][i].promStatus == "Declined at Challenge Session" || rowDB["data"][i].promStatus == "Approved at Challenge Session under Spec. Cons." || rowDB["data"][i].promStatus == "Approved at Challenge Session with Justification" || rowDB["data"][i].promStatus == "Approved at Challenge Session") && rowDB["data"][i].sortHRO == rowCollection["HRO"] && rowDB["data"][i].staffCat == "Staff") {
            nCount = nCount + 1;
        }
        else{}
    }
    for(var i = 0; i < rowDB["data"].length; i++) {
        if ((rowDB["data"][i].promStatus == "Approved at Challenge Session under Spec. Cons." || rowDB["data"][i].promStatus == "Approved at Challenge Session with Justification" || rowDB["data"][i].promStatus == "Approved at Challenge Session" || rowDB["data"][i].promStatus == "Declined at Challenge Session") && rowDB["data"][i].sortHRO == rowCollection["HRO"] && rowDB["data"][i].staffCat == "Staff") {
            mCount = mCount + 1;
        }
        else{}
    }
    if (confirm("Confirm Submission!")) {
        var count = 0;
        for(var i = 0; i < rowDB["data"].length; i++) {
            if ((rowDB["data"][i].promStatus == "Approved by Manager" || rowDB["data"][i].promStatus == "Approved by Manager with Justification" || rowDB["data"][i].promStatus == "Approved by Manager under Spec. Cons.") && rowDB["data"][i].sortHRO == rowCollection["HRO"] && rowDB["data"][i].staffCat == "Staff" && rowDB["data"][i].genCriteria == "YES") {
            count = count + 1;
            }
            else{}
        }
        if (count == 0) {
            ...
            var toPromote = "YES";
            //var trackStatus = "J";
            hroApproveRow (rowCollection["HRO"],"YES","Approved by Manager","Approved at Challenge Session",toPromote,"JA");
            hroApproveRow (rowCollection["HRO"],"YES","Approved by Manager with Justification","Approved at Challenge Session with Justification",toPromote,"J");
            hroApproveRow (rowCollection["HRO"],"YES","Approved by Manager under Spec. Cons.","Approved at Challenge Session under Spec. Cons.",toPromote,"J");
            ...
            if ((count + mCount) == nCount) {
                ...
                var to = "";
                for(var i = 0; i < rowDB["data"].length; i++) {
                    if ((rowDB["data"][i].sortHRO == rowCollection["HRO"] && rowDB["data"][i].staffCat == "Staff" && (rowDB["data"][i].promStatus == "Approved by Manager" || rowDB["data"][i].promStatus == "Approved by Manager with Justification" || rowDB["data"][i].promStatus == "Approved by Manager under Spec. Cons.") && rowDB["data"][i].genCriteria == "YES") || (rowDB["data"][i].sortHRO == rowCollection["HRO"] && (rowDB["data"][i].promStatus == "Approved at Challenge Session under Spec. Cons." || rowDB["data"][i].promStatus == "Approved at Challenge Session with Justification" || rowDB["data"][i].promStatus == "Approved at Challenge Session"))) {
                        sNo = sNo + 1;
                        var newPers = sNo + '. ' + rowDB["data"][i].persName + ' (' + rowDB["data"][i].refIndic + ')\n';
                        myTable = myTable + newPers;
                    }
...
function gmApprove2() {
    ...
    if (confirm("Confirm Submission!")) {
        var count = 0;
        for(var i = 0; i < rowDB["data"].length; i++) {
            if ((rowDB["data"][i].promStatus == "Approved by Manager" || rowDB["data"][i].promStatus == "Approved by Manager under Spec. Cons." || rowDB["data"][i].promStatus == "Approved by Manager with Justification") && rowDB["data"][i].sortHRO == rowCollection["HRO"] && rowDB["data"][i].staffCat == "Staff") {
            count = count + 1;
            }
            else{}
        }
        if (count == 0) {
            ...
            var toPromote = "YES";
            //var trackStatus = "J";
            hroApproveRow2 (rowCollection["HRO"],"Approved by Manager","Approved at Challenge Session",toPromote,"JA");
            hroApproveRow2 (rowCollection["HRO"],"Approved by Manager with Justification","Approved at Challenge Session with Justification",toPromote,"J");
            hroApproveRow2 (rowCollection["HRO"],"Approved by Manager under Spec. Cons.","Approved at Challenge Session under Spec. Cons.",toPromote,"J");
            ...
            if ((count + mCount) == nCount) {
                ...
                var to = "";
                for(var i = 0; i < rowDB["data"].length; i++) {
                    if ((rowDB["data"][i].sortHRO == rowCollection["HRO"] && rowDB["data"][i].staffCat == "Staff" && (rowDB["data"][i].promStatus == "Approved by Manager" || rowDB["data"][i].promStatus == "Approved by Manager with Justification" || rowDB["data"][i].promStatus == "Approved by Manager under Spec. Cons.")) || (rowDB["data"][i].sortHRO == rowCollection["HRO"] && (rowDB["data"][i].promStatus == "Approved at Challenge Session under Spec. Cons." || rowDB["data"][i].promStatus == "Approved at Challenge Session with Justification" || rowDB["data"][i].promStatus == "Approved at Challenge Session"))) {
                        sNo = sNo + 1;
                        var newPers = sNo + '. ' + rowDB["data"][i].persName + ' (' + rowDB["data"][i].refIndic + ')\n';
                        myTable = myTable + newPers;
                    }
